...
class PagerMediator (
    ...
    private val db: DataBaseProject
    ...
    override suspend fun load(
        ...
        try {
            val response = apiServices.getRepos(page = page.toString())
            val isEndOfList = false
            db.withTransaction {
                if (loadType == LoadType.REFRESH) {
                    db.getRoomModelDao().deleteAll()
                    db.getKeysDao().deleteAll()
                }
                val prevKey = if (page == 1) null else page - 1
                val nextKey = if (isEndOfList) null else page + 1
                val modelsList = (response.body() as GitRepModel).items
                val keys = modelsList.map{
                    RemoteKey(it.id, prevKey = prevKey, nextKey = nextKey)
                }
                db.getKeysDao().insertAll(keys)
                db.getRoomModelDao().insertAll(modelsList.map { it.gitRepListInfo2RoomModel() })
            }
    ...
    private suspend fun getRemoteKeyClosestToCurrentPosition(state: PagingState<Int,RoomModel>): RemoteKey? {
        return state.anchorPosition?.let { position ->
            state.closestItemToPosition(position)?.id?.let { repoId ->
                db.getKeysDao().remoteKeysId(repoId)
            }
        }
    }
    ...
    private suspend fun getLastRemoteKey(state: PagingState<Int, RoomModel>): RemoteKey? {
        return state.pages
            ...
            ?.data?.lastOrNull()
            ?.let { cat -> db.getKeysDao().remoteKeysId(cat.id) }
    }
